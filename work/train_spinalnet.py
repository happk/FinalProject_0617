...
def train():
    ...
    with fluid.dygraph.guard(place = fluid.CUDAPlace(0)):
        ...
        if train_parameters["continue_train"]:
            model, _ = fluid.dygraph.load_dygraph(train_parameters["save_spinalnet"])
            net.load_dict(model)
        ...
        for epoch_num in range(epoch_num):
            ...
            for batch_id, data in enumerate(train_reader()):
                ...
                if batch_id % 40 == 0:
                   ...
                   all_eval_rewards.append(epoch_acc)
                   net.train()
                   if  epoch_acc > best_acc:
                       fluid.dygraph.save_dygraph(net.state_dict(), train_parameters["save_spinalnet"])
                       best_acc = epoch_acc
                       logger.info("model saved at epoch {}, best accuracy is {}".format(epoch_num, best_acc))
        logger.info("Final loss: {}".format(avg_loss.numpy()))
        np.savez('result/spinalnet_result.npz', all_train_rewards=all_train_rewards, all_eval_rewards=all_eval_rewards)
